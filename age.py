...
def education_graph(data):
	...
	datafile=datafile[['industry','year','quarter','agegrp','EarnS','EmpS']]
	datafile=datafile[datafile['agegrp']!='A00']
	datafile=datafile[datafile['agegrp']!='A01']
	datafile=datafile[datafile['agegrp']!='A08']
		...
		for year in range(2000,2015,1):
			...
			for quarter in range(1,5,1):
				...
				try:
					...
					young2=quarter_data[quarter_data['agegrp']=='A02']['EmpS'].sum()
					young3=quarter_data[quarter_data['agegrp']=='A03']['EmpS'].sum()
					young4=quarter_data[quarter_data['agegrp']=='A04']['EmpS'].sum()
					young=young2+young3+young4
					total_earn=quarter_data['weighted'].sum()
					average_earn=total_earn/total_emp
					young_share=young/total_emp
				except:
					average_earn=0
					young_share=0
					total_emp=0

				national_list.append([year,quarter,industry,average_earn,young_share,total_emp])
...
def education_graph_hires(data):
	...
	# which will be the numerator of the average earnings calculation.
	industries=set(datafile['industry'])
	datafile=datafile[['industry','year','quarter','agegrp','EarnHirAS','HirAS']]
	datafile=datafile[datafile['agegrp']!='A00']
	datafile=datafile[datafile['agegrp']!='A01']
	datafile=datafile[datafile['agegrp']!='A08']
	datafile['year']=datafile['year'].astype(float)
		...
		for year in range(2000,2015,1):
			...
			for quarter in range(1,5,1):
				...
				try:
					...
					young2=quarter_data[quarter_data['agegrp']=='A02']['HirAS'].sum()
					young3=quarter_data[quarter_data['agegrp']=='A03']['HirAS'].sum()
					young4=quarter_data[quarter_data['agegrp']=='A04']['HirAS'].sum()
					young=young2+young3+young4
					total_earn=quarter_data['weighted'].sum()
					average_earn=total_earn/total_emp
					young_share=young/total_emp
